...
(function(){
  /* =====================================================================
     ...
     SHARED CHECKS     isDraftComplete (used by matchups.js + shares.js
                       ...
                       findMatchupPair (used by matchups.js + live.js)
     ROUTER            render()
     ...
     SCHEDULE 2026     BYE_WEEKS, SCHEDULE_2026, getMatchup (used by
                       planner.js + live.js via EZL)
  ...
  function render(){
    ...
    if(state.view === 'liveHub') return window.Live.render();
  }
    ...
    return `
      <div class="topbar">
        ...
        <div class="topbar-actions">
          ...
          ${state.leagues && state.leagues.length ? `<button class="btn btn-ghost" id="btn-livehub">📺 Live Hub</button>` : ''}
  ...
  function bindTopbar(){
    ...
    const lh = document.getElementById('btn-livehub');
    if(lh) lh.addEventListener('click', ()=>{ state.view='liveHub'; render(); });
  ...
  // Finds this week's head-to-head pair (my matchup row + opponent's) from
  // a league detail's cached matchupsWeek1. Originally private to
  // matchups.js; pulled up here once live.js needed the exact same lookup,
  // rather than a second copy of the same few lines.
  ...
  // Originally private to planner.js; moved here and exposed via EZL once
  // live.js needed the exact same schedule/lookup rather than a duplicate.
  const SCHEDULE_2026 = {
